...
const excelSafaMashaer = require('../../controllers/excelSafaMashaerVacant.js');
...
const downloadFile = async (req, res) => {
    await safamashaer.findAll({
        ...
        const capacity = await safamashaerrooms.findAll({
            where: {
                capacity: {
                    [Op.gt]: 0
                }
...
router
    .route('/safamashaervacant/file')  
    .get(downloadFile);
    
router
    .route('/vacantsafamashaer/uploadfile')
    .post(upload.single("file"), excelSafaMashaer.uploadVacantFile);

router
    .route("/searchvacant/safamashaer")
    .get(async (req, res) => {
        const { q } = req.query;
            const data = await safamashaerrooms.findAll({
                where: {
                    room: {
                        [Op.like]: q
                    }
...
router
    .route('/safamashaervacant/update/:id')
    .put(async (req, res) => {
        const { id } = req.params;
        const record = req.body;
        console.log(record)
        await safamashaerrooms.findByPk(id,
            {
                where:
                {
                    id: id
