...
const excelSawady2 = require('../../controllers/excelSawady2Vacant.js');
...
const downloadFile = async (req, res) => {
    await sawady2.findAll({
        ...
        const capacity = await sawady2rooms.findAll({
            where: {
                capacity: {
                    [Op.gt]: 0
                }
...
router
    .route('/sawady2vacant/file')  
    .get(downloadFile);
    
router
    .route('/vacantsawady2/uploadfile')
    .post(upload.single("file"), excelSawady2.uploadVacantFile);

router
    .route("/searchvacant/sawady2")
    .get(async (req, res) => {
        const { q } = req.query;
            const data = await sawady2rooms.findAll({
                where: {
                    room: {
                        [Op.like]: q
                    }
...
router
    .route('/sawady2vacant/update/:id')
    .put(async (req, res) => {
        const { id } = req.params;
        const record = req.body;
        console.log(record)
        await sawady2rooms.findByPk(id,
            {
                where:
                {
                    id: id
