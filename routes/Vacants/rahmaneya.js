...
const excelRahmaneya = require('../../controllers/excelRahmaneyaVacant.js');
...
const downloadFile = async (req, res) => {
    await rahmaneya.findAll({
        ...
        const capacity = await rahmaneyarooms.findAll({
            where: {
                capacity: {
                    [Op.gt]: 0
                }
...
router
    .route('/rahmaneyavacant/file')  
    .get(downloadFile);
    
router
    .route('/vacantrahmaneya/uploadfile')
    .post(upload.single("file"), excelRahmaneya.uploadVacantFile);

router
    .route("/searchvacant/rahmaneya")
    .get(async (req, res) => {
        const { q } = req.query;
            const data = await rahmaneyarooms.findAll({
                where: {
                    room: {
                        [Op.like]: q
                    }
...
router
    .route('/rahmaneyavacant/update/:id')
    .put(async (req, res) => {
        const { id } = req.params;
        const record = req.body;
        console.log(record)
        await rahmaneyarooms.findByPk(id,
            {
                where:
                {
                    id: id
