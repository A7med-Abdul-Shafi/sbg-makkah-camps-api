...
const excelBatawy1 = require('../../controllers/excelBatawy1Vacant.js');
...
const downloadFile = async (req, res) => {
    await batawy1.findAll({
        ...
        const capacity = await batawy1rooms.findAll({
            where: {
                capacity: {
                    [Op.gt]: 0
                }
...
router
    .route('/batawy1vacant/file')  
    .get(downloadFile);
    
router
    .route('/vacantbatawy1/uploadfile')
    .post(upload.single("file"), excelBatawy1.uploadVacantFile);

router
    .route("/searchvacant/batawy1")
    .get(async (req, res) => {
        const { q } = req.query;
            const data = await batawy1rooms.findAll({
                where: {
                    room: {
                        [Op.like]: q
                    }
...
router
    .route('/batawy1vacant/update/:id')
    .put(async (req, res) => {
        const { id } = req.params;
        const record = req.body;
        console.log(record)
        await batawy1rooms.findByPk(id,
            {
                where:
                {
                    id: id
