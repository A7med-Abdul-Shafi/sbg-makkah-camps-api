...
const excelMorgan2Bathaa = require('../../controllers/excelMorgan2BathaaVacant.js');
...
const downloadFile = async (req, res) => {
    await morgan2bathaa.findAll({
        ...
        const capacity = await morgan2bathaarooms.findAll({
            where: {
                capacity: {
                    [Op.gt]: 0
                }
...
router
    .route('/morgan2bathaavacant/file')  
    .get(downloadFile);
    
router
    .route('/vacantmorgan2bathaa/uploadfile')
    .post(upload.single("file"), excelMorgan2Bathaa.uploadVacantFile);

router
    .route("/searchvacant/morgan2bathaa")
    .get(async (req, res) => {
        const { q } = req.query;
            const data = await morgan2bathaarooms.findAll({
                where: {
                    room: {
                        [Op.like]: q
                    }
...
router
    .route('/morgan2bathaavacant/update/:id')
    .put(async (req, res) => {
        const { id } = req.params;
        const record = req.body;
        console.log(record)
        await morgan2bathaarooms.findByPk(id,
            {
                where:
                {
                    id: id
