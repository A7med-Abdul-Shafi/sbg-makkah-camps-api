...
const excelElfateh = require('../../controllers/excelElfatehVacant.js');
...
const downloadFile = async (req, res) => {
    await elfateh.findAll({
        ...
        const capacity = await elfatehrooms.findAll({
            where: {
                capacity: {
                    [Op.gt]: 0
                }
...
router
    .route('/elfatehvacant/file')  
    .get(downloadFile);
    
router
    .route('/vacantelfateh/uploadfile')
    .post(upload.single("file"), excelElfateh.uploadVacantFile);

router
    .route("/searchvacant/elfateh")
    .get(async (req, res) => {
        const { q } = req.query;
            const data = await elfatehrooms.findAll({
                where: {
                    room: {
                        [Op.like]: q
                    }
...
router
    .route('/elfatehvacant/update/:id')
    .put(async (req, res) => {
        const { id } = req.params;
        const record = req.body;
        console.log(record)
        await elfatehrooms.findByPk(id,
            {
                where:
                {
                    id: id
