...
const excelEwaaa = require('../../controllers/excelEwaaaVacant.js');
...
const downloadFile = async (req, res) => {
    await ewaaa.findAll({
        ...
        const capacity = await ewaaarooms.findAll({
            where: {
                capacity: {
                    [Op.gt]: 0
                }
...
router
    .route('/ewaaavacant/file')  
    .get(downloadFile);
    
router
    .route('/vacantewaaa/uploadfile')
    .post(upload.single("file"), excelEwaaa.uploadVacantFile);

router
    .route("/searchvacant/ewaaa")
    .get(async (req, res) => {
        const { q } = req.query;
            const data = await ewaaarooms.findAll({
                where: {
                    room: {
                        [Op.like]: q
                    }
...
router
    .route('/ewaaavacant/update/:id')
    .put(async (req, res) => {
        const { id } = req.params;
        const record = req.body;
        console.log(record)
        await ewaaarooms.findByPk(id,
            {
                where:
                {
                    id: id
